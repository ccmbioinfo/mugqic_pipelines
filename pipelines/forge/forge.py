...
cp \\
  ...
  {report_file}""".format(
    ...
    def individual_VQSR_VCF_intersect(self):
	"""
	...
	3) Find the intersection of "sample.PASSvqsr.vcf" file with the "sample.hc.vcf" file of each sample. The latter file 
	...
	for sample in self.samples:
		...
		# intersection of PASSvqsr with .hc.vcf file
	...
	and find the intersection of this file with the original alignment/Sample.hc.vcf file generated by GenotypeGVCF
	...
	We don't want to the QUAL and INFO values (e.g. DP, MQ, ReadPosRankSum, MQRankSum, etc.) to be affected by multi-sample processing        
        ...
        for sample in self.samples:
	    # First VCF file either comes from the "individual_VQSR_GVCF_intersect"
            #input_VCF1 = os.path.join("variants", sample.name, sample.name + ".intersect.vcf")
            input_VCF1 = os.path.join("variants", sample.name, sample.name + ".intersect.vcf")

	    # Second VCF file is 'alignment/sample.hc.vcf' 
    ...
    # this step seems to be hanging when run in parallel with varscan (may be due to both steps using samtools.mpileup but with different parameters)
    # so for now it's set so that it will start only after varscan step finishes
    def samtoolsVarcal(self):
		...
		Job(input_files=[variant_file_prefix + ".varscan.vcf"], command="touch " + variant_file_prefix + ".varscanDone.txt"),
    ...
    def freebayes(self):
		...
		Job(command="rm " + os.path.join("alignment", sample.name) + "/*recal.REF_* " + freebayes_folder)
    ...
    def steps(self):
	    ...
	    self.individual_VQSR_VCF_intersect,
