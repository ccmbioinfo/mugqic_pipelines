...
### Report Generation
Reports can be generated by re-running the command line with the `--report` flag. This can be easily done by modifying the `generate_qsub.sh` script and adding the option somewhere in the parameter list. 

The report will contain the minimum set of data that can describe the results of the pipeline. When the report is generated, it can be found at `report/`, under your designated output directory. The report provides the metrics collection from the readset level as well as the metrics from the sample level. While the read output of interest is from the differential analysis results, no visualization has been made. Instead, a copy of the differential analysis results made available to download.
