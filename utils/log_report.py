...
import argparse, collections, subprocess, re, os
...
class JobLog:
    ...
    def format_field(self, field):
        """
        Lookup the field in self, and format the result for displaying in a table

        :param field: one of the fields in __slots__
        :return: string representation of self's field
        """
        if hasattr(self, field):
            if field == 'start_date' or field == 'end_date':
                # Always format dates as OUTPUT_DATE_FORMAT
                return str(getattr(self, field).strftime(OUTPUT_DATE_FORMAT))
            else:
    ...
    # Return a tab-delimited list in order of __slots__
    # Eg:
    # 123   456 trimmomatic 789 SUCCESS 0   0   ...
    def __str__(self):
        return '\t'.join(self.format_field(field) for field in JobLog.__slots__)
...
def filter_by_success(job_logs, keep_successful, keep_unsuccessful):
    ...
    """
    successful_job = lambda log: log.MUGQIC_exit_status == 0

    if keep_successful:
        job_logs = [log for log in job_logs if successful_job(log)]
    elif keep_unsuccessful:
        job_logs = [log for log in job_logs if not successful_job(log)]
...
def populate_job_logs(job_list_file, keep_successful, keep_unsuccessful):
    """
...
def summarize_jobs(job_logs):
    """
    Compute a summary of the job_logs, like start date, highest memory, etc.

    :param job_logs: list of JobLogs
    :return: start date, end date,
             lowest memory JobLog, highest memory JobLog,
             shortest JobLog, longest JobLog,
             and the number of jobs that are successful, active, inactive, and failed,
    """
    # Sort the jobs that have a 'mem' attribute
    sorted_by_memory = sorted(filter(lambda log: hasattr(log, 'mem'), job_logs), key = lambda log: log.mem)
    # Sort the jobs that have a 'walltime' attribute
    sorted_by_time = sorted(filter(lambda log: hasattr(log, 'walltime'), job_logs), key = lambda log: log.walltime)

    return min([log.start_date for log in job_logs if hasattr(log, 'start_date')]), \
           max([log.end_date for log in job_logs if hasattr(log, 'end_date')]), \
           sorted_by_memory[0], \
           sorted_by_memory[-1], \
           sum(log.status == 'SUCCESS' for log in job_logs), \
...
def get_log_text_report(job_logs):
    """
    Unassigned fields are given the value 'N/A'
    :return: report string
    """
    start_date, end_date, lowest_memory_job, highest_memory_job,\
        num_successful, num_active, num_inactive, num_failed,\
        shortest_job, longest_job = summarize_jobs(job_logs)
...
# Lowest memory job: {lowest_name} ({lowest_mem})
# Highest memory job: {highest_name} ({highest_mem})

'''.format(num_jobs = len(job_logs), successful = num_successful, active = num_active, inactive = num_inactive, failed = num_failed,
           exec_time = exec_time,
           shortest_name = shortest_job.job_name, shortest_duration = str(shortest_job.walltime),
           longest_name = longest_job.job_name, longest_duration = str(longest_job.walltime),
           lowest_name = lowest_memory_job.job_name, lowest_mem = lowest_memory_job.mem,
           highest_name = highest_memory_job.job_name, highest_mem = highest_memory_job.mem)
...
'''
    ...
    table_body = '\n'.join([str(log) for log in job_logs])

    return summary + table_header + table_body
...
def parse_args():
    ...
    :return: args from ArgumentParser
    '''
    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument('job_file', help='Path to file containing jobs and associated info')
    arg_parser.add_argument('-s', '--success', help='Show successful jobs only', action='store_true')
    arg_parser.add_argument('-nos', '--nosuccess', help='Show unsuccessful jobs only i.e. failed or uncompleted jobs', action='store_true')
    # arg_parser.add_argument('-v', '--verbose', help='Display some messages about what the script is doing', action='store_true')

    args = arg_parser.parse_args()

    return args.job_file, args.success, args.nosuccess


def main():
    job_list_file, success_option, no_success_option = parse_args()

    job_logs = populate_job_logs(job_list_file, success_option, no_success_option)

    print(get_log_text_report(job_logs))
