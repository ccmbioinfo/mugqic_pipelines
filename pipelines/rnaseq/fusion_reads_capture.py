...
class FusionCapture(common.Illumina):
	"""
	RNA-Seq Pipeline
	================
...
########### Fusion functions ###########
	def defuse(self):
		...
		for readset in self.readsets:
			...
			out_dir = os.path.join("fusions", "defuse", readset.sample.name)
			defuse_job = defuse.defuse(fastq1, fastq2, out_dir)
			job = concat_jobs([
				Job(command="mkdir -p " + out_dir),
				defuse_job
			], name="defuse." + readset.name)
	...
	def fusionmap(self):
		...
		for readset in self.readsets:
			...
			out_dir = os.path.join("fusions", "fusionmap", readset.sample.name)
	...
	def tophat2(self):
		...
		for readset in self.readsets:
			...
			out_dir = os.path.join(self.output_dir, "fusions", "tophat2", readset.sample.name)
			tophat2_job = tophat2.tophat2(fastq1, fastq2, out_dir)
			job = concat_jobs([
				Job(command="mkdir -p " + out_dir),
				tophat2_job
			], name="tophat2." + readset.name)
	...
	def integrate(self):
		...
		jobs = []
		for readset in self.readsets:
			input_dir = os.path.join("fusions", "tophat2", readset.sample.name)
	...
	def ericscript(self):
		...
		for readset in self.readsets:
			...
			out_dir = os.path.join("fusions", "ericscript", readset.sample.name)
			ericscript_job = ericscript.ericscript(fastq1, fastq2, out_dir)
			job = concat_jobs([
				Job(command="mkdir -p " + out_dir),
				Job(command="rm -r " + out_dir),
				ericscript_job
			], name="ericscript." + readset.name)
	...
	def convert_fusion_results_to_cff(self):
		...
		for sample in self.samples:
			...
			integrate_result = os.path.join("fusions", "integrate", sample.name, "breakpoints.tsv")
			...
			for contrast in self.contrasts:
				...
				if sample_type:
					...
					break	
			if not sample_type:
				raise Exception("Error: sample " + sample_name + " not found in design file " + design_file)
				
			for tool, result_file in tool_results:
				job = cff_convertion.cff_convert(sample.name, result_file, sample_type, disease_name, tool, out_dir)
				job.command = job.command.strip()
				job_list.append(job)
		job = concat_jobs(job_list, name="cff_convertion." + sample.name)
		jobs.append(job)
		return jobs
	...
	def merge_and_reannotate_cff_fusion(self):
		...
		job = concat_jobs([
			reann_job	
		], name="merge_and_reannotate_cff_fusion." + sample.name)
